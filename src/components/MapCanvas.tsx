...
export function MapCanvas({ pack, activeTool = 'select', selection, route, calibrationPoints = [], interactive = false, showGraph = false, currentCheckpointId, targetCheckpointId, onMapClick, onNodeActivate, onObjectSelect, onMoveNode }: Props) {
  ...
  return <div className="map-frame" aria-label="Floor plan map">
    ...
    <svg className={`map-overlay tool-${activeTool}`} viewBox="0 0 1000 1000" preserveAspectRatio="none"
      ...
      {pack.checkpoints.map((item) => {
        ...
        return <g key={item.id} className={`map-marker checkpoint ${selected ? 'selected' : ''}${role}`} transform={`translate(${toSvg(position.x)} ${toSvg(position.y)})`}
